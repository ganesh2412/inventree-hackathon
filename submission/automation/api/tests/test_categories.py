...
# ─────────────────────────────────────────────
# TC_API_044 – TC_API_050: GET /api/part/category/
# ─────────────────────────────────────────────

BASE_CAT_URL = "/api/part/category/"


@pytest.fixture
def cat_url(base_url):
  return f"{base_url}{BASE_CAT_URL}"
...
class TestCategoriesGet:
  ...
  def test_TC_API_047_get_category_no_auth_returns_401(self, base_url):
    """TC_API_047: GET /api/part/category/ without auth returns 401."""
    import requests
    r = requests.get(f"{base_url}{BASE_CAT_URL}")
...
# ─────────────────────────────────────────────
# TC_API_051 – TC_API_060: POST /api/part/category/
# ─────────────────────────────────────────────
...
class TestCategoriesPost:
  ...
  def test_TC_API_051_create_category_returns_201(self, api_client, cat_url):
    ...
    r = api_client.post(cat_url, json={"name": "AutoTestCat", "description": "Test category"})
  ...
  def test_TC_API_053_create_category_with_parent(self, api_client, cat_url, test_category_id):
    ...
    r = api_client.post(cat_url, json={"name": "ChildCat", "parent": test_category_id})
    assert r.status_code == 201
  ...
  def test_TC_API_054_create_category_response_has_pk(self, api_client, cat_url):
    ...
    r = api_client.post(cat_url, json={"name": "PkCat"})
    assert r.status_code == 201
    assert "pk" in r.json()
    api_client.delete(f"{cat_url}{r.json()['pk']}/")

  def test_TC_API_055_create_category_no_auth_returns_401(self, base_url):
    """TC_API_055: POST without auth returns 401."""
    import requests
    r = requests.post(f"{base_url}{BASE_CAT_URL}", json={"name": "UnAuthCat"})
    assert r.status_code == 401
  ...
  def test_TC_API_056_create_duplicate_category_name(self, api_client, cat_url):
    ...
    r1 = api_client.post(cat_url, json={"name": "DupCat"})
    assert r1.status_code == 201
    r2 = api_client.post(cat_url, json={"name": "DupCat"})
  ...
  def test_TC_API_057_create_category_with_description(self, api_client, cat_url):
    ...
    r = api_client.post(cat_url, json={"name": "DescCat", "description": "A description"})
    assert r.status_code == 201
  ...
  def test_TC_API_058_patch_category_name(self, api_client, cat_url):
    ...
    r = api_client.post(cat_url, json={"name": "PatchMe"})
    cat_id = r.json()["pk"]
    patch_r = api_client.patch(f"{cat_url}{cat_id}/", json={"name": "Patched"})
    assert patch_r.status_code == 200
    assert patch_r.json()["name"] == "Patched"
    api_client.delete(f"{cat_url}{cat_id}/")

  def test_TC_API_059_delete_category_returns_204(self, api_client, cat_url):
    """TC_API_059: DELETE category returns 204."""
    r = api_client.post(cat_url, json={"name": "DeleteCat"})
    cat_id = r.json()["pk"]
